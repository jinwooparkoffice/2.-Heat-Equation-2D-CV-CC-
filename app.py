...
def flush_print(*args, **kwargs):
    try:
        # 1차 시도: 그냥 출력 (터미널이 UTF-8이면 성공)
        print(*args, **kwargs)
        sys.stdout.flush()
    except UnicodeEncodeError:
        try:
            # 2차 시도: 인코딩 문제 방지를 위해 ASCII 문자로 변환
            new_args = []
            for arg in args:
                if isinstance(arg, str):
                    # cp949로 인코딩 불가능한 문자는 ? 등으로 대체
                    # Windows 콘솔에서는 cp949가 기본일 수 있음
                    new_args.append(arg.encode('cp949', 'replace').decode('cp949'))
                else:
                    new_args.append(arg)
            ...
            try:
                ...
                for arg in args:
                    if isinstance(arg, str):
                        ...
                        new_args.append(str(arg).encode('ascii', 'replace').decode('ascii'))
                print(*new_args, **kwargs)
                sys.stdout.flush()
            except:
                pass
        ...
        for sid in to_remove:
            ...
            if os.path.exists(result_file):
                try:
                    ...
                    flush_print(f"[CLEANUP] Deleted old result file: {sid}")
                except Exception as e:
                    flush_print(f"[WARN] Failed to delete result ({sid}): {e}")
...
def periodic_cleanup_worker():
    ...
    while cleanup_thread_running.is_set():
        try:
            ...
            flush_print(f"[ERROR] Periodic cleanup error: {e}")
...
def cleanup_on_exit():
    ...
    flush_print("[STOP] Process shutting down... cleaning up")
    ...
    flush_print("[SUCCESS] ThreadPoolExecutor shutdown")
    
    # 마지막으로 한 번 더 정리 실행
    cleanup_old_progress()
    flush_print("[SUCCESS] Cleanup complete")
...
def handle_general_exception(e):
    ...
    # 상세한 오류 정보 출력
    flush_print(f"[ERROR] Global Exception Handler: {error_type}: {error_msg}")
    flush_print("=" * 80)
    flush_print("Detail Error Info:")
    flush_print(error_traceback)
    flush_print("=" * 80)
    
    # 디버그 모드일 때만 traceback 포함
    include_traceback = os.environ.get('FLASK_DEBUG', 'False').lower() == 'true'
...
def check_password():
    ...
    try:
        ...
        flush_print(f"[WARN] Password check error: {e}")
...
def get_progress(session_id):
    ...
    try:
        ...
        flush_print(f"[WARN] Progress check error (session_id={session_id}): {e}")
        flush_print(traceback.format_exc())
        return jsonify({
            'progress': 0,
...
def cancel_simulation(session_id):
    ...
    try:
        with cancel_lock:
            ...
            flush_print(f"[STOP] Simulation cancellation requested: {session_id}")
        
        # 진행률 업데이트
        with progress_lock:
            if session_id in progress_store:
                progress_store[session_id]['message'] = 'Canceling...'
        ...
        flush_print(f"[WARN] Cancel request error: {e}")
        return jsonify({
            'success': False,
...
def simulate():
    ...
    try:
        ...
        try:
            ...
            flush_print(f"[WARN] JSON parsing error: {json_error}")
            flush_print(traceback.format_exc())
            return jsonify({
                'success': False, 
        ...
        try:
            ...
            flush_print(f"[WARN] Progress init error: {lock_error}")
            flush_print(traceback.format_exc())
            return jsonify({
                'success': False,
        ...
        def run_simulation():
            try:
                ...
                error_msg = f"Simulation Worker Error: {str(e)}"
                flush_print(f"[ERROR] {error_msg}")
                flush_print(f"Traceback:\n{traceback.format_exc()}")
                with progress_lock:
        ...
        try:
            ...
            flush_print(f"[WARN] Simulation submit error: {submit_error}")
            flush_print(traceback.format_exc())
            # 진행률 저장소에서 세션 제거
            with progress_lock:
                if session_id in progress_store:
        ...
        error_msg = f"Unexpected error during simulation request: {str(e)}"
        flush_print(f"[ERROR] {error_msg}")
        flush_print(traceback.format_exc())
        return jsonify({
            'success': False,
...
def _simulate_worker(session_id, data):
    ...
    try:
        ...
        flush_print(f"=== Simulation Request (session_id: {session_id}) ===")
        
        if not data:
            raise ValueError("No request data.")
        
        flush_print(f"Request Keys: {list(data.keys())}")
        ...
        flush_print(f"Layer Count: {len(layer_names)}")
        ...
        # =========================
        # 열원(면적당) 모델
        # 에너지 보존 법칙: 총 전력 입력 = 광출력 + 열원
        # 총 열원 = V*J - V_photon*J*EQE
        # V_photon = hc/(qλ): 광자 에너지에 해당하는 전압
        # 광출력(외부): V_photon*J*EQE 는 열원에서 제외
        # =========================
        # 상수 (SI)
        ...
        # 면적당 광출력(외부)
        optical_out_A = current_density_A_per_m2 * V_photon * eqe  # W/m²
        # 총 열원: 총 전력 입력 - 광출력
        Q_A = voltage * current_density_A_per_m2 - optical_out_A  # W/m²
        # Perovskite에 열원 적용 (총 열원을 Perovskite에만 적용)
        perovskite_heat_A = Q_A
        ...
        # t_eval 생성: 초반을 촘촘하게, 후반을 성글게 (제곱근 스케일 사용)
        # solve_ivp 요구사항: t_eval의 모든 값이 t_span [t_start, t_end] 범위 내에 있어야 함
        # 제곱근 스케일: 초반에 더 많은 포인트 배치 (온도 변화가 빠른 초반을 정확히 포착)
        
        try:
            # 제곱근 스케일: t = t_start + (t_end - t_start) * (u^2)
            # 여기서 u는 [0, 1] 범위의 균등 간격
            # 이렇게 하면 초반이 촘촘하고 후반이 성글게 됨
            u = np.linspace(0, 1, n_time_points)
            t_eval = t_start + (t_end - t_start) * (u ** 2)
            ...
            # 계산 실패 시 균등 간격 사용
            flush_print(f"[WARN] Time point generation failed: {e}, using linspace")
        ...
        if min_t_eval < t_start - tolerance:
            flush_print(f"[WARN] t_eval min < t_start: min={min_t_eval:.6e}, t_start={t_start:.6e}")
            t_eval = np.clip(t_eval, t_start, t_end)
            t_eval = np.sort(t_eval)
        
        if max_t_eval > t_end + tolerance:
            flush_print(f"[WARN] t_eval max > t_end: max={max_t_eval:.6e}, t_end={t_end:.6e}")
            t_eval = np.clip(t_eval, t_start, t_end)
            t_eval = np.sort(t_eval)
        ...
        assert len(t_eval) == n_time_points, f"t_eval length mismatch: {len(t_eval)} != {n_time_points}"
        assert np.all(t_eval >= t_start), f"t_eval < t_start: min={np.min(t_eval):.6e}, t_start={t_start:.6e}"
        assert np.all(t_eval <= t_end), f"t_eval > t_end: max={np.max(t_eval):.6e}, t_end={t_end:.6e}"
        assert np.all(np.diff(t_eval) >= 0), "t_eval is not monotonically increasing"
        
        flush_print(f"[OK] t_eval generated: {len(t_eval)} points, range=[{np.min(t_eval):.6e}, {np.max(t_eval):.6e}], t_span=[{t_start:.6e}, {t_end:.6e}]")
        ...
        flush_print(f"=== Z-Direction Grid Generation ===")
        ...
        perov_q_vol = perovskite_heat_A / L_perovskite  # W/m³
        ...
        flush_print(f"=== Heat Source Info ===")
        flush_print(f"Voltage V = {voltage:.4f} V, V_photon = {float(V_photon):.4f} V")
        flush_print(f"Total Power Input = {voltage * current_density_A_per_m2:.2f} W/m^2")
        flush_print(f"Optical Output (excluded) = {optical_out_A:.2f} W/m^2")
        flush_print(f"Total Heat Source Q_A = {Q_A:.2f} W/m^2")
        flush_print(f"perovskite_heat_A = {perovskite_heat_A:.2f} W/m^2")
        flush_print(f"L_perovskite = {L_perovskite*1e9:.2f} nm")
        flush_print(f"Source Nodes (non-zero): {num_source_nodes}")
        ...
        # 초기 진행률 설정
        update_progress(5, 'Generating grid...')
        
        # PDE 시스템 정의 (RHS 최적화: Flat indexing 사용)
        def pde_system(t, T_flat):
            # 취소 플래그 확인 (주기적으로 확인)
            if is_cancelled():
                flush_print(f"[STOP] Simulation cancelled (session_id: {session_id}, t={t:.3f} s)")
            ...
            if t - last_print_time[0] >= 5.0:  # 5초마다 업데이트 (오버헤드 최소화)
                ...
                message = f"Running... t = {t:.3f} s ({t/t_end*100:.1f}%), T[0, 0] = {T_center:.2f} K"
                flush_print(message)
            ...
            if t == t_start or abs(t - t_start) < 1e-6:
                T_center = T_flat[0]
                flush_print(f"=== Solver Start (t={t}, t_end={t_end:.1f} s) ===")
                flush_print(f"T[0, 0] = {T_center:.2f} K")
                if source_flat[0] != 0:
                    flush_print(f"Heat Source: source_flat[0] = {source_flat[0]:.6f} K/s")
                update_progress(10, f'Solver started... (t_end = {t_end:.1f} s)')
        ...
        flush_print(f"=== Solver Running ===")
        flush_print(f"Grid Size: {Nr} x {Nz} = {Nr*Nz} nodes")
        flush_print(f"Time Span: {t_start:.6f} ~ {t_end:.1f} s")
        flush_print(f"Time Points: {len(t_eval)}")
        
        update_progress(15, f'Solver running... (Grid: {Nr}x{Nz})')
        
        # 솔버 실행 전 취소 플래그 확인
        if is_cancelled():
            flush_print(f"[STOP] Simulation cancelled (before solver, session_id: {session_id})")
            update_progress(0, 'Cancelled')
            with progress_lock:
                progress_store[session_id] = {
                    'progress': 0,
        ...
        try:
            ...
            if "취소" in str(cancel_error) or "cancel" in str(cancel_error).lower():
                flush_print(f"[STOP] Simulation cancelled (session_id: {session_id})")
                with progress_lock:
                    progress_store[session_id] = {
                        'progress': 0,
                        'message': 'Cancelled',
                ...
                error_msg = f"Solver Error: {str(cancel_error)}"
                flush_print(f"[ERROR] {error_msg}")
                flush_print(f"Traceback:\n{traceback.format_exc()}")
                update_progress(0, f'Solver Error: {str(cancel_error)}')
                raise ValueError(error_msg) from cancel_error
        except Exception as solver_error:
            error_msg = f"Solver Error: {str(solver_error)}"
            flush_print(f"[ERROR] {error_msg}")
            flush_print(f"Traceback:\n{traceback.format_exc()}")
            update_progress(0, f'Solver Error: {str(solver_error)}')
            raise ValueError(error_msg) from solver_error
        
        elapsed_time = time.time() - start_time
        flush_print(f"=== Solver Completed (Elapsed: {elapsed_time:.1f} s) ===")
        update_progress(95, f'Processing results... (Elapsed: {elapsed_time:.1f} s)')
        
        if not sol.success:
            error_msg = f"Solver Failed: {sol.message}"
            flush_print(f"[ERROR] {error_msg}")
            update_progress(0, f'Solver Failed: {sol.message}')
        ...
        # 디버깅: 온도 값 확인
        flush_print(f"=== Solver Result Debug ===")
        flush_print(f"Success: {sol.success}")
        flush_print(f"T_result shape: {T_result.shape}")
        flush_print(f"Min Temp: {np.min(T_result):.2f} K")
        flush_print(f"Max Temp: {np.max(T_result):.2f} K")
        flush_print(f"Mean Temp: {np.mean(T_result):.2f} K")
        flush_print(f"T_ambient: {T_ambient:.2f} K")
        flush_print(f"Init Temp T0[0, 0]: {T0[0, 0]:.2f} K")
        flush_print(f"Final Temp T_result[0, 0, 0]: {T_result[0, 0, 0]:.2f} K")
        flush_print(f"Final Temp T_result[0, 0, -1]: {T_result[0, 0, -1]:.2f} K")
        
        # 온도가 비정상적으로 낮은지 확인
        if np.min(T_result) < 100:
            flush_print(f"[WARN] Min temp too low! ({np.min(T_result):.2f} K)")
        if np.max(T_result) < T_ambient:
            flush_print(f"[WARN] Max temp lower than ambient!")
        ...
        if T_2d_raw.shape[0] > max_r_points or T_2d_raw.shape[1] > max_z_points:
            flush_print(f"[INFO] Downsampling temp_2d: {T_2d_raw.shape} -> max ({max_r_points}, {max_z_points})")
            # 균등 간격으로 다운샘플링
        ...
        try:
            ...
            for i in range(1, min(end_idx, max_idx)):
                if i >= len(thickness_layers_nm_original):
                    raise IndexError(f"Index Error: i={i}")
                active_layer_boundaries_nm.append(float(active_layer_boundaries_nm[-1] + thickness_layers_nm_original[i]))
            
            flush_print(f"[INFO] Active layer boundaries calculated: {len(active_layer_boundaries_nm)-1} layers")
        except (IndexError, ValueError) as e:
            raise ValueError(f"Layer boundary calculation error: {str(e)}") from e
        ...
        flush_print(f"=== Profile 1: Temp vs z at r=0 (Final) ===")
        flush_print(f"Data Size: {len(temp_profile_r0_z)} z points")
        flush_print(f"Temp Range: {min(temp_profile_r0_z):.2f} ~ {max(temp_profile_r0_z):.2f} K")
        flush_print(f"=== Temp vs z,time at r=0 (For Sheet1) ===")
        flush_print(f"Data Size: {len(temp_profile_r0_z_time)} z points x {len(temp_profile_r0_z_time[0]) if temp_profile_r0_z_time and len(temp_profile_r0_z_time) > 0 else 0} time points")
        ...
        if perovskite_mid_idx is not None and perovskite_mid_idx < T_result.shape[1] and perovskite_mid_idx >= 0:
            ...
            flush_print(f"=== Profile 2: Temp vs r at z=perovskite_mid (Final) ===")
            flush_print(f"perovskite_mid_idx: {perovskite_mid_idx}, z: {z_nm[perovskite_mid_idx]:.2f} nm")
            flush_print(f"Data Size: {len(temp_profile_z_perovskite_r)} r points")
            flush_print(f"Temp Range: {min(temp_profile_z_perovskite_r):.2f} ~ {max(temp_profile_z_perovskite_r):.2f} K")
        else:
            temp_profile_z_perovskite_r = []
            flush_print(f"[WARN] Invalid perovskite_mid_idx. Skipping Profile 2.")
        
        # 3. z=perovskite 중점, r=0에서 시간에 따른 온도 프로파일 (이미 계산됨: perovskite_center_temp)
        flush_print(f"=== Profile 3: Temp vs time at z=perovskite_mid, r=0 ===")
        flush_print(f"Data Size: {len(perovskite_center_temp)} time points")
        if len(perovskite_center_temp) > 0:
            flush_print(f"Temp Range: {min(perovskite_center_temp):.2f} ~ {max(perovskite_center_temp):.2f} K")
        ...
        # 디버깅: 반환 전 온도 값 확인
        flush_print(f"=== Result Check ===")
        if len(perovskite_center_temp) > 0:
            flush_print(f"perovskite_center_temp[0]: {perovskite_center_temp[0]:.2f} K")
            flush_print(f"perovskite_center_temp[-1]: {perovskite_center_temp[-1]:.2f} K")
        if temperature_2d and len(temperature_2d) > 0 and len(temperature_2d[0]) > 0:
            flush_print(f"temperature_2d[0][0]: {temperature_2d[0][0]:.2f} K")
        
        try:
            update_progress(100, 'Done!')

            # Glass t-T: 최하단(z=0, 공기 경계)에서의 온도 (r=0)
            glass_bottom_idx = 0
            glass_bottom_temp = T_result[0, glass_bottom_idx, :].tolist() if Nz > 0 else []
            glass_bottom_z_nm = float(z_nm[glass_bottom_idx]) if len(z_nm) > glass_bottom_idx else None
            ...
            result_data = {
                ...
                'glass_bottom_temp': np.array(glass_bottom_temp),
                'glass_bottom_z_nm': glass_bottom_z_nm
            }
            
            # 결과를 디스크에 저장 (npz 형식)
            result_file = os.path.join(RESULTS_DIR, f"{session_id}.npz")
            np.savez_compressed(result_file, **result_data)
            flush_print(f"[INFO] Result saved to disk: {result_file}")
            ...
            result_summary = {
                ...
                'glass_bottom_temp': convert_to_python_type(glass_bottom_temp),
                'glass_bottom_z_nm': convert_to_python_type(glass_bottom_z_nm)
            }
            
            flush_print(f"=== Preparing Result ===")
            flush_print(f"Result Size: time={len(result_summary.get('time', []))}, temp_2d={len(result_summary.get('temperature_2d', []))}x{len(result_summary.get('temperature_2d', [])[0]) if result_summary.get('temperature_2d') else 0}")
            ...
            flush_print(f"=== Result Stored in Progress Store ===")
        except Exception as result_error:
            error_msg = f"Result Processing Error: {str(result_error)}"
            flush_print(f"[ERROR] {error_msg}")
            flush_print(f"Traceback:\n{traceback.format_exc()}")
            update_progress(0, f'Result Error: {str(result_error)}')
            raise
        ...
        error_traceback = traceback.format_exc()
        flush_print(f"=== Simulation Error ===")
        flush_print(f"Error Type: {type(e).__name__}")
        flush_print(f"Message: {str(e)}")
        flush_print(f"Traceback:\n{error_traceback}")
