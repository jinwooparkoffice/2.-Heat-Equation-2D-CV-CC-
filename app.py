...
def _simulate_worker(session_id, data):
    ...
    try:
        ...
        try:
            ...
            # 계산 실패 시 균등 간격 사용
            flush_print(f"⚠️ 시간 포인트 생성 실패: {e}, 균등 간격으로 대체")
        ...
        flush_print(f"=== 열원 정보 ===")
        flush_print(f"전압 V = {voltage:.4f} V, V_photon = {float(V_photon):.4f} V")
        flush_print(f"총 전력 입력 = {voltage * current_density_A_per_m2:.2f} W/m²")
        flush_print(f"광출력 (excluded) = {optical_out_A:.2f} W/m²")
        flush_print(f"총 열원 Q_A = {Q_A:.2f} W/m²")
        flush_print(f"perovskite_heat_A = {perovskite_heat_A:.2f} W/m²")
