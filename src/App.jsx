...
const DEFAULT_VALUES = {
  ...
  rho_layers: [2500, 7140, 1200, 1200, 1200, 2700, 1100, 2330],
  c_p_layers: [1000, 280, 1500, 1500, 1500, 900, 1800, 700],
...
function App() {
          ...
          if (progressData.result && progressData.progress >= 100) {
            ...
            try {
              ...
              const convertedData = {
                ...
                // Glass t-T (r=0): glass 최하단(z=0, 공기 경계) 온도 시계열 (켈빈 -> 섭씨)
                glass_bottom_temp: result.glass_bottom_temp ? result.glass_bottom_temp.map(kelvin => kelvinToCelsius(kelvin)) : null,
                glass_bottom_z_nm: result.glass_bottom_z_nm ?? null
              }
  ...
  const handleSaveExcel = () => {
    ...
    try {
      const { time, temperature_center, perovskite_center_temp, temp_profile_z_perovskite_r, r_mm, z_profile_nm, temp_profile_r0_z, temp_profile_r0_z_time, z_profile_nm_sampled, glass_ito_boundary_nm, layer_boundaries_nm, temperature_2d, position_active_nm, glass_bottom_temp, glass_bottom_z_nm } = simulationResult
      ...
      // Sheet6: Glass t-T (r=0, glass 최하단(z=0, 공기 경계))
      const sheet6Data = []
      const bottomNm = glass_bottom_z_nm
      sheet6Data.push([
        '시간 (s)',
        `Glass bottom (air boundary) (z=${bottomNm !== undefined && bottomNm !== null ? Number(bottomNm) : 'N/A'} nm) (°C)`
      ])

      if (time && glass_bottom_temp) {
        const n = time.length
        for (let i = 0; i < n; i++) {
          sheet6Data.push([
            Number(time[i]),
            i < glass_bottom_temp.length ? Number(glass_bottom_temp[i]) : ''
          ])
        }
      }
      ...
      XLSX.utils.book_append_sheet(wb, ws6, 'Glass t-T')
